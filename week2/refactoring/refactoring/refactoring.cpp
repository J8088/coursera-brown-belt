...
Json::Document XmlToJson(const Xml::Document& doc) {
	...
	for (const auto& xml_node : doc.GetRoot().Children()) {
		result.emplace_back(map<string, Json::Node>{
			{"category", Json::Node(xml_node.AttributeValue<string>("category")) },
			{ "amount", Json::Node(xml_node.AttributeValue<int>("amount")) }
		});
	}
...
Xml::Document JsonToXml(const Json::Document& doc, string root_name) {
	...
	for (const auto& json_node : doc.GetRoot().AsArray()) {
		root.AddChild(Xml::Node("spend", {
		  {"category", json_node.AsMap().at("category").AsString()},
		  {"amount", to_string(json_node.AsMap().at("amount").AsInt())},
		}));
	}
