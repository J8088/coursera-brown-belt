...
public:
	struct WriteAccess : lock_guard<mutex> {
		...
		WriteAccess(const K& key, Segment& segment)
			: lock_guard(segment.m)
			, ref_to_value(segment.data[key])
		{
		}
	};
	...
	struct ReadAccess : lock_guard<mutex> {
		...
		ReadAccess(const K& key, const Segment& segment)
			: lock_guard(segment.m)
			, ref_to_value(segment.data.at(key))
		{
		}
	};
	...
	WriteAccess operator[](const K& key) {
		return { key, segments[GetIndex(key)] };
	}
	...
	bool Has(const K& key) const {
		auto& segment = segments[GetIndex(key)];
		lock_guard g(segment.m);
		return segment.data.count(key) > 0;
	}

	MapType BuildOrdinaryMap() const {
		MapType whole_map;
		for (const Segment& segment : segments) {
			lock_guard<mutex> guard(segment.m);
			whole_map.insert(segment.data.begin(), segment.data.end());
		}
		return whole_map;
	}
...
private:
	size_t GetIndex(const K& key) const {
		return hasher(key) % segments.size();
	}
};
